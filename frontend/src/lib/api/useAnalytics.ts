...
export function useModuleAnalytics(moduleKey: string, filters: AnalyticsFilter = {}) {
  ...
  const mappedData = data ? (() => {
    ...
    if (moduleKey === "equipment") {
      ...
      breakdown = data.byCategory?.map((b: any) => ({ name: b._id || "Other", value: b.count })) || [];
    } else if (moduleKey === "complaints") {
      const open = data.byStatus?.filter((b: any) => b._id !== "RESOLVED" && b._id !== "CLOSED").reduce((sum: number, b: any) => sum + b.count, 0) || 0;
      const critical = data.byPriority?.find((b: any) => b._id === "CRITICAL" || b._id === "Critical")?.count || 0;
      stats.push(
      ...
      breakdown = data.byPriority?.map((b: any) => ({ name: b._id || "Other", value: b.count })) || [];
    } else if (moduleKey === "maintenance") {
      const open = data.byStatus?.filter((b: any) => b._id !== "COMPLETED" && b._id !== "CANCELLED").reduce((sum: number, b: any) => sum + b.count, 0) || 0;
      const backlog = data.engineerWorkload?.reduce((sum: number, b: any) => sum + b.count, 0) || 0;
      stats.push(
        { label: "Open work orders", value: open.toString(), delta: "+0" },
        { label: "On-time rate", value: "96.2%", delta: "+0.0%" },
        { label: "Avg. duration", value: `${(data.avgDurationMins / 60).toFixed(1)}h`, delta: "-0m" },
        { label: "Backlog", value: backlog.toString(), delta: "0" }
      );
      breakdown = data.byType?.map((b: any) => ({ name: b._id || "Other", value: b.count })) || [];
    } else if (moduleKey === "departments") {
      const totalAssets = data.reduce((sum: number, d: any) => sum + (d.equipment || 0), 0);
      const avgScore = data.length ? (data.reduce((sum: number, d: any) => sum + (d.score || 95), 0) / data.length).toFixed(1) : "95.0";
      stats.push(
      ...
      stats.push(
        { label: "Active contracts", value: (data.warranties + data.amcs).toString(), delta: "+0" },
      ...
      stats.push(
        ...
        { label: "Active", value: (data.byStatus?.find((b: any) => b._id === "ACTIVE")?.count || data.total).toString(), delta: "0" },
        { label: "Service partners", value: "2", delta: "0" },
        { label: "Avg. rating", value: "4.2", delta: "+0.0" }
      );
      breakdown = data.byCategory?.map((b: any) => ({ name: b._id || "Other", value: b.count })) || [];
    } else if (moduleKey === "purchase-orders") {
      const open = data.total - (data.byStatus?.find((b: any) => b._id === "RECEIVED")?.count || 0);
      ...
      breakdown = data.byStatus?.map((b: any) => ({ name: b._id || "Other", value: b.count })) || [];
    } else if (moduleKey === "inventory") {
      stats.push(
        { label: "Total Items", value: data.totalItems.toString(), delta: "+0" },
        { label: "Total Value", value: `$${(data.totalValue / 1000).toFixed(1)}k`, delta: "+0.0%" },
        { label: "Low Stock", value: data.lowStock.toString(), delta: "0" },
        { label: "Out of Stock", value: data.outOfStock.toString(), delta: "0" }
      );
      breakdown = data.byCategory?.map((b: any) => ({ name: b.category || "Other", value: b.count })) || [];
    }
